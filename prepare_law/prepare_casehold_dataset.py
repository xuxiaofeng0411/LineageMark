...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Prepare a 35k CaseHOLD legal multiple-choice subset for causal LM full fine-tuning."
    )
...
def load_and_sample_casehold(args):
    ...
    required_columns = {"context", "endings", "label"}
    ...
    def has_valid_casehold_fields(example):
        ...
        label = parse_label(example.get("label"))
        return (
            len(context) >= args.min_context_chars
            and len(endings) == len(ANSWER_LABELS)
            and all(endings)
...
def format_dataset(sampled, tokenizer, args):
    ...
    def format_batch(examples):
        ...
        for idx in range(batch_size):
            context = clean_text(examples["context"][idx])
            endings = normalize_endings(examples["endings"][idx])
