...
def _iter_watermark_coordinates(args, layer_id, weights, W_mask, layer_name):
    ...
    for row_id in range(row_count):
        ...
        row_values = weights[row_id]
        row_mask = None if mask is None else mask[row_id]
        for col_id in range(col_count):
            if row_mask is not None and not row_mask[col_id]:
                continue
            if row_values[col_id] == 0:
                continue
            yield row_id, col_id
...
def extract_from_weight_bitflip(args, layer_id, original_weight, ones, zeros, is_robust,
    ...
    mask = _mask_to_bool_array(W_mask)
    row_count, col_count = original_weight.shape
    ...
    for row_id in range(row_count):
        ...
        for col_id in range(col_count):
            ...
            use_weight, insert_position, insert_bit_position = get_ellmark_bit_mapping(
                args, value, chunk_length
            )
            if not use_weight:
                continue
