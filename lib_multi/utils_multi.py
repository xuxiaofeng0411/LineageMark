...
def is_ellmark_watermark_row(args, layer_id, row_id):
    gamma_row = max(1, args.hidden_size // 4)
    row_key = int(hashlib.md5((args.password + str(layer_id) + str(row_id)).encode()).hexdigest(), 16)
    return row_key % gamma_row == 0


def get_ellmark_bit_mapping(args, weight, chunk_length):
    position_bits = get_position_bits(weight, getattr(args, "position_num", 12))
    rng_seed = int(hashlib.md5((args.password + position_bits).encode()).hexdigest(), 16)
    rng = np.random.default_rng(seed=rng_seed)
    xi = max(1, int(args.xi))
    if int(rng.integers(0, xi)) % xi != 0:
        return False, None, None
    return True, int(rng.integers(0, xi)) % xi, int(rng.integers(0, chunk_length)) % chunk_length
